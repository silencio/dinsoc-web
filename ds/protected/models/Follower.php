...
/**
 * This is the model class for table "dinsoc.follower".
 *
 * The followings are the available columns in table 'dinsoc.follower':
...
class Follower extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_id, follower_id', 'required'),
			array('user_id, follower_id', 'length', 'max'=>10),
			array('created_at', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('user_id, follower_id, created_at', 'safe', 'on'=>'search'),
		);
	}
